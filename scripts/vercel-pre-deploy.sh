...
EOF
    ...
    # Try to capture intent from recent changes
    RECENT_CHANGES="$PROJECT_ROOT/.recent-changes.txt"
    cat > "$RECENT_CHANGES" << EOF
...
CHANGED FILES:
$(git diff --cached --name-only 2>/dev/null || echo "No files changed")

DIFF PREVIEW (First 100 lines):
$(git diff --cached | head -100 2>/dev/null || echo "No diff available")
EOF
...
4. Recent Changes: $RECENT_CHANGES (intent and context)
...
1. **UNDERSTAND THE INTENT** - Read $RECENT_CHANGES to understand:
